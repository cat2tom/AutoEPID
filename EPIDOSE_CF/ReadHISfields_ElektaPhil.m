...
% Define X and Y coordinates of EPID image
EpidPixel = 0.04; %pixel pitch in cm
profcol = 512; %central column index
profrow = 512; %central row index
Xr = ((1:1024) - profcol); % crossplane vector
XR = Xr*EpidPixel; %crossplane vector - cm
Yr = ((1:1024) - profrow); %inplane vector
YR = Yr*EpidPixel; %inplane vector - cm
...
imagesc(DISum);figure(gcf);
figure, plot(YR, DISum(:,512));
figure, plot(XR, DISum(512,:));

Profile=DISum(512,:);

SampleCAX = zeros(20,20);
SampleCAX = DISum(502:522,502:522); %samples integrated pixel values
MeanPixel = mean(mean(SampleCAX))%mean of central 20x20 pixels
StanDev = std2(SampleCAX);%standard deviation of central 20x20 pixels
PercentStanDev = 100*StanDev/MeanPixel
