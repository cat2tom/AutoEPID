...
function handles=AUTOIMRT13_Callback3_CF_PDF_noReg(hObject,handles)
...
for k=1:length(log_structure_list)
          ...
          if strcmp(tps,'Xio')
              if strcmp( handles.output_option, 'EPID Dose Image+AutoReport')
                [gamma_image_file_name,profile_image_file_name]=profileGammaAnalysisElektaPin2_noReg(tps_file,epid_his_file,DTA,tol,PSF,pixel_dose_factor,coll_angle);              
          ...
          if strcmp(tps,'Pinnacle')
              if strcmp( handles.output_option, 'EPID Dose Image+AutoReport')
                [gamma_image_file_name,profile_image_file_name,npass,gmean,npass2,gmean2,beam_name2]=profileGammaAnalysisElektaPin2_noReg(tps_file,epid_his_file,DTA,tol,PSF,pixel_dose_factor,coll_angle);   
              end
              
              % convert to epid dose only.
