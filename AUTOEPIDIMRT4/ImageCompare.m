...
p1.addParamValue('xlabel', 'x (mm)', @ischar);
p1.addParamValue('ylabel', 'y (mm)', @ischar);
...
try
    ...
    p2.addParamValue('xlabel', 'x (mm)', @ischar);
    p2.addParamValue('ylabel', 'y (mm)', @ischar);
...
if isempty(xvals)
    ...
    set(handles.gamma_pushbutton,'enable','on');
end
...
% get maximum value x and y

[y,x]=ind2sub(size(image1),find(image1==max(image1(:))));
...
x0value = xval(round(x));

y0value =yval(round(y));
...
ylabel(handles.SubImage2_axes,'Dose (cGy)');
ylabel(handles.SubImage1_axes,'Dose (cGy)');
...
% ylabel(handles.Image2_axes,'Dose (cGy)')
% set(handles.image1_panel,'title',['X Profile: y = ' num2str(y0value) ' mm']);
% set(handles.image2_panel,'title',['Y Profile: x = ' num2str(x0value) ' mm']);
