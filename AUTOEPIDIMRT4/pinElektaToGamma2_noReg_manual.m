function [gamma_result_file_name,numpass3b,avg3b,numpass2b,avg2b,beam_name2]=pinElektaToGamma2_noReg_manual(pin_tps_file, epid_his_file,DTA,tol,PSF,pixel_dose_factor,coll_angle)
...
% No registration version just for commissioning purpose.
...
% image1=100*image1./max(image1(:)); disable the registration.
...
%    image1=image1*0.95;
    xp=xgrid;
    yp=ygrid;
    %thresh=0.1; % changed from 10 to 5%.
    
    thresh=10;
    
%     thresh=8; % this is optimized threshold for really 10%.
%     dosetol=tol+4;
    dosetol=tol;
%     dta=DTA;
    dta=DTA;
...
[gmap,npass,gmean,ncheck] = GammaNoReg(image1,image2,xp,yp,dosetol,dta,thresh,rad_pix);


% added 2mm/2% gamma calculation


[gmap2,npass2,gmean2,ncheck2] = GammaNoReg(image1,image2,xp,yp,dosetol2,dta2,thresh,rad_pix);
...
if numpass2b>numpass3b
    
    numpass2b=(numpass3b*100-1.5)/100;
...
% final statistics final results to get same gamma as in gamma images
